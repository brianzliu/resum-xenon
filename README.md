...
After `process_xenon_original_vars.py`, run `convert_csv_to_h5.py`. Hardcode the path to the `only1` / `only2` / `original_vars (1 and 2)` folder inside `convert_csv_to_h5.py` accordingly. 

### CNP Training/Prediction
For the CNP, make sure before training you are referencing the correct file path to train on; depending on the dataset you're using, change the following block in `settings.yaml`:
